...
const ProfileScreen = () => {
      ...
      if (candidateData) {
        ...
        const [
          ...
          supabase
            .from("APPLICATIONS")
            .select(
              `id, candidate_id, job_id, resume_id, status, applied_at, JOB_POSTINGS ( id, title, location, working_model, job_type, COMPANIES ( name, logo_url ) )`,
            )
            .eq("candidate_id", candidateId)
          ...
          supabase
            .from("SAVED_JOBS")
            .select(
              `candidate_id, job_id, saved_at, JOB_POSTINGS ( id, title, location, working_model, job_type, COMPANIES ( name, logo_url ) )`,
            )
            .eq("candidate_id", candidateId)
  ...
  // Reload tab applied/saved
  const refreshJobTabs = async () => {
    if (!user?.id) return;
    await fetchCandidateProfileData(user.id);
  };
  ...
  const handleUpdateResume = async () => {
    try {
      ...
      const result = await DocumentPicker.getDocumentAsync({
        ...
        copyToCacheDirectory: true, // ✅ Bắt buộc trên Android
      });
      ...
      // 2. Đọc file thành blob
      const response = await fetch(doc.uri);
      const blob = await response.blob();

      if (blob.size === 0) throw new Error("File rỗng, vui lòng chọn lại.");

      // 3. Tạo đường dẫn file trên Storage
      const fileExt = doc.name.split(".").pop();
      const filePath = `${candidateInfo.id}/cv_${Date.now()}.${fileExt}`;

      // 4. Upload lên Supabase Storage bằng SDK (thay vì fetch thủ công)
      const { data: storageData, error: storageError } = await supabase.storage
        .from("resumes") // ✅ Tên bucket trên Supabase
      ...
      // 7. Cập nhật state hiển thị CV mới nhất
      setResume(resumeRow);
      Alert.alert("✅ Thành công", `Đã tải lên CV: ${doc.name}`);
    } catch (error) {
          ...
          <View style={styles.listContainer}>
            {appliedJobs.map((item) => (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
                onPress={() => {
                  const jobId = item.job_id ?? item.JOB_POSTINGS?.id;
                  if (!jobId) return;
